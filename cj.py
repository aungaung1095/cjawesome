...
def colorful_log(proxy, device_id, message_type, message_content, is_sent=False, mode=None):
    ...
    log_message = f"{Fore.WHITE}[{timestamp}] {Fore.MAGENTA}[Proxy: {proxy}] {Fore.CYAN}[Device ID: {device_id}] {action_color}[{message_type}] {color}{message_content} {mode_color}[{mode}]"
    print(log_message)
...
async def connect_to_wss(socks5_proxy, user_id, mode):
    ...
    colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="INITIALIZATION", message_content=f"User Agent: {random_user_agent}", mode=mode)
    has_received_action, is_authenticated = False, False

    while True:
        try:
            await asyncio.sleep(random.randint(1, 10) / 10)
            custom_headers = {"User-Agent": random_user_agent, "Origin": "chrome-extension://lkbnfiajjmbhnfledhphioinpickokdi" if mode == "extension" else None}
            ...
            async with proxy_connect(uri, proxy=proxy, ssl=ssl_context, server_hostname="proxy.wynd.network", extra_headers=custom_headers) as websocket:
                async def send_ping():
                    while True:
                        if has_received_action:
                            send_message = json.dumps({"id": str(uuid.uuid5(uuid.NAMESPACE_DNS, socks5_proxy)), "version": "1.0.0", "action": "PING", "data": {}})
                            colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="SENDING PING", message_content=send_message, is_sent=True, mode=mode)
                ...
                while True:
                    if is_authenticated and not has_received_action:
                        colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="AUTHE", message_content="Waiting for HTTP_REQUEST" if mode == "extension" else "OPEN_TUNNEL", mode=mode)
                    response = await websocket.recv()
                    message = json.loads(response)
                    colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="RECEIVED", message_content=json.dumps(message), mode=mode)
                    if message.get("action") == "AUTH":
                        ...
                        colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="AUTHENTICATING", message_content=json.dumps(auth_response), is_sent=True, mode=mode)
                        await websocket.send(json.dumps(auth_response))
                        is_authenticated = True
                    elif message.get("action") in ["HTTP_REQUEST", "OPEN_TUNNEL"]:
                        has_received_action = True
                        request_data = message["data"]
                        headers = {"User-Agent": custom_headers["User-Agent"], "Content-Type": "application/json; charset=utf-8"}
                        async with aiohttp.ClientSession() as session:
                            async with session.get(request_data["url"], headers=headers) as api_response:
                                ...
                                colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="OPENING PING ACCESS", message_content=json.dumps(http_response), is_sent=True, mode=mode)
                                await websocket.send(json.dumps(http_response))
                    elif message.get("action") == "PONG":
                        pong_response = {"id": message["id"], "origin_action": "PONG"}
                        colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="SENDING PONG", message_content=json.dumps(pong_response), is_sent=True, mode=mode)
                        await websocket.send(json.dumps(pong_response))
        except Exception as e:
            colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="ERROR", message_content=str(e), mode=mode)
            await asyncio.sleep(5)

            # Increment failed attempts counter
            failed_attempts[socks5_proxy] = failed_attempts.get(socks5_proxy, 0) + 1

            # Remove proxy if failed attempts reach 3
            if failed_attempts[socks5_proxy] >= 15:
                with open('proxy_list.txt', 'r') as file:
                    local_proxies = file.readlines()
                local_proxies = [line.strip() for line in local_proxies if not line.strip() == socks5_proxy]
                with open('proxy_list.txt', 'w') as file:
                    file.writelines(f"{line}\n" for line in local_proxies)
                colorful_log(proxy=socks5_proxy, device_id=device_id, message_type="REMOVING DEAD PROXY", message_content="Proxy removed after 15 failed attempts", mode=mode)
                break
